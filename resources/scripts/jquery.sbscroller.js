...
(function( $ ){
  ...
  $.fn.sbscroller = function( options ) {  
	...
	function setSlider($scrollpane){
		...
		difference = Math.round((difference / 64));
		...
		if(difference>0)//if the scrollbar is needed, set it up...
		{
		       ...
		       var contentHeight = $scrollpane.find('.scroll-content').height();
		       contentHeight = Math.round(contentHeight / 64);
			   ...
			   $scrollpane.find('.slider-vertical').slider({
				  ...
				  slide: function(event, ui) {
				      var topValue = -1 * (64 * (difference - ui.value));
